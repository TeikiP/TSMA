...
int main(int argc, char* argv[]) {
  ...
  while (read_samples(infile, new_buffer, sfinfo.channels) == 1) {
    ...
    double ind_peaks[FRAME_SIZE / 2];
    double amp_peaks[FRAME_SIZE / 2];
    unsigned int nb_peaks = 0;
    const double seuil_amp = 1.0;

    for (i = 1; i < FRAME_SIZE / 2 - 1; i++) {
      if (amplitude[i] > amplitude[i - 1] && amplitude[i] > amplitude[i + 1] &&
          amplitude[i] > seuil_amp) {
        ind_peaks[nb_peaks] = i;
        amp_peaks[nb_peaks] = amplitude[i];
        nb_peaks++;
      }
    }
    ...
    gnuplot_plot_xy(h, ind_peaks, amp_peaks, nb_peaks, "pics");
    sleep(1);
